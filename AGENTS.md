...
- [x] patdb_query.py（櫃台端查詢/取消綁定）- **v1.15 已完成（2026-07-10 重構）**
...
#### 2026-07-10 更新（v1.14/15）：patdb_query.py UI 流程重構

**問題 1：LINE 綁定成功但本地資料庫未寫入**
- **原因**：當時流程是「產生驗證碼 → 立刻寫入本地 → 用戶去 LINE」。若操作順序顛倒，LINE 會成功但本地沒記錄
- **解法**：改為「先確認 LINE 綁定狀態，再寫入本地」
  - 產生驗證碼時立即檢查 LINE 是否已綁定
  - 若已有 → 直接寫入本地
  - 若沒有 → 啟用「確認 LINE 綁定」按鈕，等用戶回來點擊確認

**問題 2：訊息框 blocking 導致按鈕樣式變化看不見**
- **原因**：`messagebox.showinfo()` 是同步的，UI 在訊息框消失前不重繪
- **解法**：在訊息框前加 `self.root.update()` 強迫 UI 立即更新

**問題 3：視窗太小看不到「驗證碼」區塊**
- **解法**：Tab1 改用 Canvas + Scrollbar，可上下滾動

**問題 4：按鈕狀態變化不明顯**
- **解法**：初始用灰底灰字，啟用後變藍底白字粗體

**問題 5：`on_unbind_select` 出現 `recno_hash is not defined`**
- **解法**：從 `recno` 重新計算 `recno_hash`

**patdb_query.py 完整流程圖（v1.15）**
```
選 A（綁定人）→ 搜尋 → 雙擊選擇
選 B（被綁定人）→ 搜尋 → 雙擊選擇
↓
點「確認並產生驗證碼」
↓
API：POST /api/create-verify-code
↓
檢查 LINE 綁定狀態（links-by-recno-hash）
    ├─ 已有綁定 → 直接寫入本地 → 「LINE 已有有效綁定，本地記錄已儲存！」
    └─ 無綁定 → 藍色按鈕 + 紅字「等待 LINE 綁定確認：A → B」
       ↓
告知 A 去 LINE 輸入驗證碼
       ↓
A 在 LINE 輸入驗證碼 → 成功
...
點「確認 LINE 綁定」（藍色按鈕）
↓
查 LINE 連結 → 寫入本地 SQLite
↓
「LINE 綁定成功，本地記錄已儲存！」
```

**按鈕樣式對照表**
| 狀態 | 「確認並產生驗證碼」 | 「確認 LINE 綁定」 | 狀態標籤 |
|------|---------------------|------------------|---------|
| 初始 | 灰色（disabled） | 灰色按鈕 | 空白 |
| A+B 選好 | 深色（可點） | - | - |
| 驗證碼產生（LINE 未綁） | 深色 | 藍底白字（normal） | 紅字「等待 LINE 綁定確認：A → B」 |
| LINE 已綁定成功 | 深色 | 灰色（disabled） | 空白 |
