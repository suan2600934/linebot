...
async function main() {
  ...
  for (let i = 0; i < scheduleData.weeks.length; i++) {
    ...
    const westYear = 1911 + Number(scheduleData.year);
    const outputPath = path.join(__dirname, `schedule-${westYear}-${String(scheduleData.month).padStart(2,'0')}-week${i + 1}.png`);
